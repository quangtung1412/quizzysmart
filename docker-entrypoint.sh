...
TABLE_COUNT=$(mysql -h mysql -u root -prootpassword quizzysmart -e "SHOW TABLES;" 2>/dev/null | wc -l || echo "0")
...
if [ "$TABLE_COUNT" -le 1 ]; then
  ...
  if [ -d "/backups" ]; then
    ...
    if [ -n "$LATEST_BACKUP" ] && [ -f "$LATEST_BACKUP" ]; then
      ...
      # Import the SQL file
      if mysql -h mysql -u root -prootpassword quizzysmart < "$LATEST_BACKUP" 2>&1; then
        echo "[OK] Backup imported successfully!"
        
        # Verify import
        NEW_TABLE_COUNT=$(mysql -h mysql -u root -prootpassword quizzysmart -e "SHOW TABLES;" 2>/dev/null | wc -l || echo "0")
