...
const ResultsScreen: React.FC<ResultsScreenProps> = ({ questions, userAnswers, attemptId, user, onRestart }) => {
  ...
  return (
    <div className="flex flex-col items-center">
      <h2 className="text-2xl sm:text-3xl font-bold text-slate-800">Kết quả bài thi</h2>
      <p className="text-slate-500 mt-1">Chúc mừng bạn đã hoàn thành!</p>

      {/* Completion time info */}
      {completionTimeInfo && (
        <div className="mt-3 p-3 bg-slate-100 rounded-lg border border-slate-200 text-center">
      ...
      <div className="my-4 sm:my-6 w-full max-w-sm bg-slate-50 p-4 sm:p-5 rounded-xl border border-slate-200 text-center">
        <p className="text-lg text-slate-600">Điểm số của bạn</p>
        <p className={`text-5xl sm:text-6xl font-bold my-2 ${parseFloat(score) >= 80 ? 'text-green-600' :
      ...
      <div className="flex items-center gap-3 flex-wrap justify-center">
        <button
          onClick={onRestart}
          className="px-6 sm:px-8 py-2.5 sm:py-3 text-sm font-medium text-white bg-sky-600 border border-transparent rounded-md shadow-sm hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500"
        >
          Chọn bài khác
        </button>
        <button
          onClick={() => setShowDetails(!showDetails)}
          className="px-6 sm:px-8 py-2.5 sm:py-3 text-sm font-medium text-sky-700 bg-sky-100 border border-transparent rounded-md hover:bg-sky-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500"
        >
      ...
      {showDetails && (
        <div className="mt-6 sm:mt-8 w-full border-t pt-4">
          <h3 className="text-lg sm:text-xl font-semibold text-slate-700 mb-3">Xem lại bài làm</h3>
          {loading && (
          ...
          <div className="space-y-3 sm:space-y-4">
            {resultsWithCorrectAnswers.map((result, index) => {
              ...
              return (
                <div key={q.id} className="p-3 sm:p-4 bg-white rounded-lg border border-slate-200">
                  <p className="font-semibold text-slate-800 text-sm sm:text-base flex flex-wrap items-center gap-2">
